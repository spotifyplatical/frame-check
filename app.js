...
const reportBlockEl = document.getElementById("reportBlock");
const reportFpsBtnEl = document.getElementById("reportFpsBtn");

// Kept in sync on every update so the report button always reflects
// exactly what's currently on screen, without re-reading the DOM on click
...
function buildReportEmail(ctx) {
  const subject = "FrameCheck FPS report";
  const body = [
    "Sharing a real result to help calibrate FrameCheck's estimates:",
    "",
    `GPU: ${ctx.gpuName}`,
    `CPU: ${ctx.cpuName}`,
    `Resolution: ${ctx.resolution}`,
    `Render distance used: ${ctx.renderDistance} chunks`,
    `Sodium/Lithium (or similar): ${ctx.sodium ? "Yes" : "No"}`,
    `Shader: ${ctx.shaderLabel}`,
    `Target FPS entered: ${ctx.targetFPS}`,
    `FrameCheck's estimate: ${ctx.estimatedFPS}`,
    "",
    "Actual FPS I got: ",
    "Any other notes: ",
  ].join("\n");

  return `mailto:platical.corp@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}
...
function init() {
  ...
  reportFpsBtnEl.addEventListener("click", () => {
    if (!lastReportContext) return;
    window.location.href = buildReportEmail(lastReportContext);
  });
...
function updatePage() {
  ...
  reportBlockEl.hidden = false;
  lastReportContext = {
    gpuName: gpuName || "unknown",
    cpuName: cpuName || "unknown",
    resolution,
    targetFPS,
    renderDistance: result.renderDistance,
    shaderLabel: shader ? `${shader.name} (${shader.presets[0]})` : "None (vanilla)",
    sodium: sodiumInput.checked,
